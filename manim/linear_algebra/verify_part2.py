...
REQUIRED = [
    ...
    "column", "row", "null", "rank", "nullity", "self.cc(",
]
...
def main() -> None:
    ...
    # Mathematical checkpoints that guard against the previously found errors.
    assert "(3, 5)" not in source
    assert r"\vec w=2\vec u+\vec v=\begin{bmatrix}3\\4\end{bmatrix}" in source
    assert "ax.c2p(1,2), VECTOR_B" in source
    ...
    # No split corrective source should remain authoritative.
    ...
    print(f"PASS: {len(SCENES)} Part II scenes, syntax, teaching markers, numerical checkpoints, and canonical renderer verified.")
