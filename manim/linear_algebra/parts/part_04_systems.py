...
class SystemsLesson(LessonScene):
    ...
    def augmented(self, rows, scale=0.75, y=0):
        return MathTex(rows).scale(scale).to_edge(RIGHT, buff=0.28).shift(UP * y)
...
class Part4_01_AxEqualsB(SystemsLesson):
    def construct(self):
        ...
        p = self.dot(ax, (8/3, 7/3), HIGHLIGHT)
        self.play(FadeIn(p, scale=0.5))
        self.cc("A system is asking for the point that satisfies every equation at the same time. On this graph, that means the intersection of the two lines.", 3.5)
        ...
        self.cc("Those two scalar equations can be compressed into one matrix equation: A x equals b. That notation will let us solve many equations systematically.", 3.4)
...
class Part4_02_GeometricMeaning(SystemsLesson):
    def construct(self):
        ...
        self.play(FadeIn(self.dot(ax, (8/3, 7/3), HIGHLIGHT), scale=0.5))
        self.play(Write(self.eq(r"(x,y)=\left(\frac83,\frac73\right)", 0.70, -1.65)))
        self.wait(2)
...
class Part4_03_AugmentedMatrix(SystemsLesson):
    def construct(self):
        ...
        self.cc("The vertical bar is not decoration. It separates the coefficient matrix from the right-hand side vector b.", 2.8)
        self.play(Write(self.eq(r"\left[\begin{array}{cc|c}a_{11}&a_{12}&b_1\\a_{21}&a_{22}&b_2\end{array}\right]", 0.70, -0.80)))
        self.play(Write(self.eq(r"\text{same system}\iff\text{same augmented matrix data}", 0.62, -1.75)))
...
class Part4_04_ElementaryRowOperations(SystemsLesson):
    def construct(self):
        ...
        self.cc("Why are they safe? A row is an equation. Swapping equations changes only their order. Scaling a nonzero equation keeps the same set of points. Adding a multiple of one equation to another creates a logically equivalent pair.", 4.1)
...
class Part4_07_RREF(SystemsLesson):
    def construct(self):
        self.title("Part IV.7 — Reduced Row Echelon Form", "Push elimination until every pivot column becomes an identity column")
        self.play(Write(self.eq(r"\left[\begin{array}{cc|c}1&1&5\\2&-1&1\end{array}\right]", 0.90, 1.30)))
        self.cc("Gaussian elimination stops at an upper-triangular form. Reduced row echelon form goes further: eliminate above each pivot as well.", 3.2)
        self.play(Write(self.eq(r"\sim\left[\begin{array}{cc|c}1&0&2\\0&1&3\end{array}\right]", 0.95, 0.15)))
        self.cc("Now the coefficient block is the identity matrix. The solution can be read directly without back substitution.", 2.8)
        self.play(Write(self.eq(r"x=2,\qquad y=3", 0.92, -0.70)))
        self.play(Write(self.eq(r"\text{RREF makes the algebra maximally transparent}", 0.62, -1.55)))
