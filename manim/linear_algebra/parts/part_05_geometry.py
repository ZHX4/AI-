...
class Part5_05_AngleAndCauchySchwarz(GeometryLesson):
    def construct(self):
        ...
        v = self.vector(ax, (1, 1.732), VECTOR_B, r"\vec v")
        self.play(GrowArrow(u[0]), Write(u[1]), GrowArrow(v[0]), Write(v[1]))
        ...
        self.cc("Because the cosine cannot be larger than one, the dot product satisfies the Cauchy–Schwarz inequality.", 2.8)
        self.play(Write(self.eq(r"|\vec u\cdot\vec v|\leq\|\vec u\|\,\|\vec v\|", 0.88, -1.70)))
        self.cc("Equality occurs exactly when the vectors lie on the same line. This gives us a powerful way to recognize maximum alignment.", 3.0)
        self.wait(2)
...
class Part5_07_Projection(GeometryLesson):
    def construct(self):
        ...
        p = coordinate_dot(ax, (16/5, 8/5), HIGHLIGHT, 0.09)
...
class Part5_10_GramSchmidt(GeometryLesson):
    def construct(self):
        ...
        self.play(Write(self.eq(r"Q^TQ=I", 0.88, -2.90)))
        self.wait(2)
