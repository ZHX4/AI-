...
def main():
    ...
    renderer_path = ROOT / "render_part12.py"
    assert renderer_path.exists(), "Part XII renderer is missing"
    renderer = renderer_path.read_text(encoding="utf-8")
    assert 'SCRIPT = "parts/part_12_pca_final.py"' in renderer
    for scene in SCENES:
        assert scene in renderer

    print(f"PASS Part XII: {len(SCENES)} scenes, exact centering/covariance, PCA eigenpairs, projection/reconstruction, explained variance, SVD relationship, and mandatory renderer wiring verified")
